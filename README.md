...
## Read & listen (the read-along player)

Open [`readalong.html`](readalong.html) in any browser. It's a single self-contained file
that plays each chapter and **highlights the current sentence and word as it's spoken**
(karaoke style) — tap any word to jump there, with auto-scroll and a speed control. Each
chapter also shows its key takeaways as a visual card. Generated by `src/build_readalong.py`,
which re-renders the audio sentence-by-sentence so the highlight timing lines up.

## Listen (plain audio)

The rendered audio also lives in [`audio/`](audio/) as `*.mp3` (128 kbps, tagged). Play them
in order. That's it.
